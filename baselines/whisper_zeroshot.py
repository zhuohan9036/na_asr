...
def main() -> None:
    ...
    for _, row in tqdm(test_df.iterrows(), total=len(test_df), desc="Whisper inference"):
        ...
        try:
            prediction = model.transcribe_file(
                wav_path=wav_path,
            ...
            logger.warning(f"Failed to transcribe utt_id={utt_id}, wav_path={wav_path}: {e}")
    ...
    logger.info("Finished Whisper zero-shot baseline.")
